...
<x-app-layout :activeNav="'oevents'">
    <main style="max-width:800px;margin:0 auto;padding:32px 26px 60px">
        ...
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:6px">
            <h1 style="margin:0;font-size:28px;font-weight:800;letter-spacing:-.9px">Edit Ticket Type</h1>
            @if($ticketType->is_active)
                <span style="padding:5px 10px;border-radius:8px;font-size:11px;font-weight:800;text-transform:uppercase;background:var(--chip);color:var(--primary)">Active</span>
            @else
                <span style="padding:5px 10px;border-radius:8px;font-size:11px;font-weight:800;text-transform:uppercase;background:var(--surface2);color:var(--muted)">Inactive</span>
            @endif
        </div>
        <p style="margin:0 0 24px;color:var(--muted);font-size:14.5px">{{ $ticketType->name }}</p>
        ...
        <form method="POST" action="{{ route('organizer.ticket-types.update', [$event, $ticketType]) }}" style="background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:26px">
            @csrf
            @method('PUT')
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                <label style="grid-column:1/-1;display:flex;flex-direction:column;gap:7px">
                    ...
                    <input type="text" name="name" value="{{ old('name', $ticketType->name) }}" required style="min-height:48px;padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none">
                ...
                <label style="grid-column:1/-1;display:flex;flex-direction:column;gap:7px">
                    ...
                    <textarea name="description" rows="3" style="padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none;resize:vertical;line-height:1.6">{{ old('description', $ticketType->description) }}</textarea>
                </label>
                ...
                <label style="display:flex;flex-direction:column;gap:7px">
                    ...
                    <input type="number" name="quantity" value="{{ old('quantity', $ticketType->quantity) }}" min="{{ $ticketType->allocatedQuantity() }}" required style="min-height:48px;padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none">
                    <span style="font-size:12px;color:var(--muted);font-weight:600">Allocated: {{ $ticketType->allocatedQuantity() }}</span>
                ...
                <label style="display:flex;flex-direction:column;gap:7px">
                    ...
                    <input type="number" name="min_per_booking" value="{{ old('min_per_booking', $ticketType->min_per_booking) }}" min="1" required style="min-height:48px;padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none">
                </label>
                <label style="display:flex;flex-direction:column;gap:7px">
                    <span style="font-size:12.5px;font-weight:700">Max per booking *</span>
                    <input type="number" name="max_per_booking" value="{{ old('max_per_booking', $ticketType->max_per_booking) }}" min="1" required style="min-height:48px;padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none">
                </label>
                <label style="display:flex;flex-direction:column;gap:7px">
                    <span style="font-size:12.5px;font-weight:700">Sales start <span style="color:var(--muted);font-weight:600">(optional)</span></span>
                    <input type="datetime-local" name="sales_start_at" value="{{ old('sales_start_at', $ticketType->sales_start_at?->format('Y-m-d\TH:i')) }}" style="min-height:48px;padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none">
                </label>
                <label style="display:flex;flex-direction:column;gap:7px">
                    <span style="font-size:12.5px;font-weight:700">Sales end <span style="color:var(--muted);font-weight:600">(optional)</span></span>
                    <input type="datetime-local" name="sales_end_at" value="{{ old('sales_end_at', $ticketType->sales_end_at?->format('Y-m-d\TH:i')) }}" style="min-height:48px;padding:13px 15px;border:1px solid var(--border);background:var(--surface2);border-radius:12px;font-size:14.5px;outline:none">
            ...
            <div style="display:flex;gap:10px;margin-top:24px;padding-top:20px;border-top:1px solid var(--border)">
                ...
                <button type="submit" style="border:0;cursor:pointer;background:linear-gradient(135deg,var(--primary),var(--primary-dark));color:#fff;font-weight:800;font-size:14px;padding:13px 24px;border-radius:12px;min-height:48px">Update Ticket Type</button>
            </div>
        </form>
    </main>
</x-app-layout>
