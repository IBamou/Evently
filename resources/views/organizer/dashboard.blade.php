@php
    // Design dashTitle (L1593): admin role â†’ "Platform dashboard", otherwise "Welcome back, <first name>".
    ...
    // Live dashboard data (contract from the organizer dashboard controller).
    // Defensive defaults keep the view renderable if any variable is missing.
    $stats ??= [];
    $revenue ??= 0.0;
    $ticketsIssued ??= 0;
    $ticketsChecked ??= 0;
    ...
    // 4 KPI cards (real values; the design's sample deltas only appear once there is data).
    $kpis = [
        ...
        ['label' => 'Check-in rate', 'value' => $checkInRate . '%',
         'delta' => $ticketsChecked . ' scanned', 'deltaFg' => 'var(--muted)',
                ...
                @empty
                    <div style="display:grid;grid-template-columns:1.4fr 1.6fr .7fr .8fr .8fr;gap:12px;padding:40px 4px;align-items:center">
                        <div style="grid-column:1/-1;text-align:center;color:var(--muted);font-size:13.5px;font-weight:600">
                            No bookings yet
                            <a href="{{ route('organizer.events.index') }}" style="margin-left:8px;color:var(--primary);font-weight:700;text-decoration:none">Browse bookings</a>
                        </div>
                    </div>
                @endforelse
            </div>
